...
document.addEventListener('DOMContentLoaded', () => {
    console.log("LinkaVel Game Engine Starting...");
    ...
    // フィールドのセットアップ (テスト配置)
    // テスト用に m002 (指揮官) を真ん中に出しておく
    summonCardToField("m002", 1); 
...
function initDeck(recipeKey) {
    ...
    document.getElementById('ply-deck-count').innerText = GAME_STATE.player.deck.length;
}
...
function advancePhase() {
    ...
    if (nextIdx >= GAME_STATE.phases.length) {
        nextIdx = 0; // END -> DRAW (Next Turn)
        GAME_STATE.turn++;
        console.log(`--- Turn ${GAME_STATE.turn} Start ---`);
        // ターン開始時の処理（ドローなど）はここに記述
        drawCard(1); 
    }
